...
for (jj in 1:R) {
  ...
  for (bb in 1:B) {
    ...
    y_boot<-matrix(Load%*%t(Ft_boot[,bb])+Resid,nrow=N,ncol=t)
    ...
    tPC1BAI1_boot[,bb]<-ts(PC1)
    Load_boot<-matrix(t(Y)%*%PC1/t,N,r)
  ...
  for (qq in 1:t) {
    CIL[qq]<-quantile(tPC1BAI1_boot[qq,],0.05)    
    CIU[qq]<-quantile(tPC1BAI1_boot[qq,],0.95)    
  }
