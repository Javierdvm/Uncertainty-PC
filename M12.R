...
tPC1BAI1<-ts(PC1)
Load<-matrix(t(Y)%*%PC1/(t^2),N,r)
Resid<-y-Load%*%t(PC1)
tPC1<-ts(PC1)
VAR2[jj]<-(colVars(tPC1))
  #OLS
  ...
  fit_ar_bias_corrected1=(t*fit$ar+1)/(t-1)
  arr[jj]<-fit_ar_bias_corrected1
  #Bootstrap AR
  for (bb in 1:B) {
    resid_boot<-sample(resid,size = t,replace = TRUE,prob = NULL)
    resamplet2 <- sample(nrow(Resid),size = N,replace = TRUE,prob = NULL)
    Resid_boot2<-Resid[resamplet2,]
    Ft_boot[1,]=tPC1[1]
    Ft_b[1]=ts(PC1)[1]
    Yt_b[1,]<-y[,1]
    for (rr in 2:t) {
     Ft_b[rr]<-fit_ar_bias_corrected1*Ft_b[rr-1]+resid_boot[rr]
     Yt_b[rr,]<-Load*Ft_b[rr]+Resid_boot2[,rr]
    }
    #BAI
    Y<-Yt_b
    RR<-Y%*%t(Y)
    ...
    values<-eR$values[c(1:r)]
    vectors<-matrix(eR$vectors[,c(1:r)],t,r)
    PC1ESTb<-sqrt(t)*vectors
    #Efecto espejo
    PC1ESTINVb<-(-1)*PC1ESTb
    Dif1<-colSums(abs(t(F)-PC1ESTb))
    Dif2<-colSums(abs(t(F)-PC1ESTINVb))
    if (Dif1<Dif2) {
      PC1b=PC1ESTb
    } else {
      PC1b=PC1ESTINVb
    ...
    tPC1BAI1b<-ts(PC1b)
    Loadb<-matrix(t(Y)%*%PC1b/(t^2),N,r)
    #Residb<-Yt_b-Loadb%*%t(PC1b)
    tPC1b<-ts(PC1b)
    fit2<-ar.ols(tPC1b,aic=FALSE,order.max=1,demean=FALSE,intercept=FALSE)
    fit_ar_bias_corrected2=(t*fit2$ar+1)/(t-1)
    if (fit_ar_bias_corrected2>1) {ar[bb]=0.99} else {
    ar[bb]<-fit_ar_bias_corrected2
    }
  #}
...
#Bootstrap Idiosincrático
  #for (bb in 1:B) {
    ...
    for (tt in 2:t) {
      Ft_boot[tt,bb]<-ar[bb]*tPC1[tt-1]+sqrt((1-ar[bb]^2))*resid[tt]
    }
    
    y_boot<-matrix(Loadb%*%t(Ft_boot[,bb])+Resid_boot,nrow=N,ncol=t)
...
#Sacando PC del bootstrap
  ...
  VAR5<-rowVars(Ft_boot)
  #Varianza Bootstrap, mse
  varF_boot[jj]<-mean(VAR5)
...
q[rrr]<-SN
phi[rrr]<-phi1
VAR3[rrr]<-colMeans(VAR1)
VAR4[rrr]<-colMeans(VAR2)
MvarF_boot[rrr]<-colMeans(varF_boot)
}
}
#hist(ar)
#print(fit$ar)
...
write.csv2(salida,"M12 b=1000 ro=0 T=150 N=200.csv")
