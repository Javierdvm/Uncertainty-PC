  ...
  for (phi1 in c(0.2,0.5,0.9)) {
    ...
    for (jj in 1:R) {
      ...
      for (bb in 1:B) {
        ...
        fitarr_boot<-sample(fitarr,size = N,replace = TRUE,prob = NULL)
        PI_boot<-matrix(0,N,N)
        diag(PI_boot)<-fitarr_boot
      ...
      for (qq in 1:t) {
       ...
       # differences_relativ[qq]<-((var_boot[qq])-((F[qq]-PC1[qq])^2))/((F[qq]-PC1[qq])^2)
      }
      MSE<-((t(F)-PC1)^2)
      RMSE[jj]=sqrt(colMeans(MSE))
      difRMS[jj]<-colMeans(differences)
      #difRMS_rel[jj]<-colMeans(differences_relativ)
   ...
   # difff_rel[rrr]<-colMeans(difRMS_rel)
...
write.csv2(salida,"M10F RO  T=100 N=200.csv")
