...
for (SN in c(10,1,0.5,0.1)){
  ...
  for (phi1 in c(0.2,0.5,0.9)) {
    ...
    for (jj in 1:R) {
      ...
      residr<-matrix(0,N,t)
      for (qq in 1:N) {
        fitr<-ar.ols(ts(Resid[qq,]),aic=FALSE,order.max=1,demean=FALSE,intercept=FALSE)
        fitarr[qq]<-fitr$ar
        residr[qq,]<-scale(fitr$resid,center=FALSE,scale=FALSE)
        residr[is.na(residr)]<-mean(resid[-1])
      }
      ...
      #Bootstrap AR Idiosincrático
      E_boot<-matrix(0,N,t)
      Et_b<-matrix(0,t,r)
      arE_boot<-matrix(0,N,B)
      ar_E<-matrix(0,B,1)
      for (qq in 1:N) {
        for (bb in 1:B) {
          residr_boot<-sample(residr[qq,],size = N,replace = TRUE,prob = NULL)
          Et_b[1]<-Resid[1]
          for (rr in 2:t) {
            Et_b[rr]<-fitarr[qq]*Et_b[rr-1]+residr_boot[rr]
          }
          fit2E<-ar.ols(ts(Et_b),aic=FALSE,order.max=1,demean=FALSE,intercept=FALSE)
          ar_E[bb]<-fit2E$ar
        }
        arE_boot[qq,]<-sample(ar_E,size = B,replace = TRUE,prob = NULL)
      }
      
      #Bootstrap Idiosincrático
      for (bb in 1:B) {
        fitarr_boot<-matrix(0,N,1)
      
        resamplet <- sample(nrow(Resid),size = t,replace = TRUE,prob = NULL)
        Resid_boot<-Resid[resamplet,]
        
        resamplet2 <- sample(nrow(residr),size = N,replace = TRUE,prob = NULL)
        residr_boot<-residr[resamplet2,]
        E_boot[1]<-Resid[1]
        
        for (tt in 2:t) {
          Ft_boot[tt,bb]<-ar_boot[bb]*tPC1[tt-1]+resid[tt]
        }
          PI_boot<-matrix(0,N,N)
          diag(PI_boot)<-arE_boot[,bb]
          for (tt in 2:t) {
          E_boot[,tt]<-PI_boot%*%Resid[,tt-1]+residr_boot[,tt]
          }
        y_boot<-matrix(Load%*%t(Ft_boot[,bb])+E_boot,nrow=N,ncol=t)
      ...
      differences_relativ<-matrix(0,t,r)
      difRMS_rel<-matrix(0,R,r)
...
write.csv2(salida,"M10 0.7 T=100 N=200.csv")
