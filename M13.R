...
library(Matrix)
 ...
 rrr=0
 for (SN in c(5,4,3,2,1,0.5,0.3,0.25,0.2)){ 
   Tau<-1/SN
   for (phi1 in c(0.2,0.5,0.8)) {
     ...
     PI<-ro*diag(N)
     U<-matrix(diag(runif(N,0.1,2)),nrow=N, ncol=N)
     for (jj in 1:R) {
       print(SN)
       print(phi1)
       print(jj)
        ...
        tPC1<-ts(PC1)
          ...
          resid<-scale(fit$resid,center=FALSE,scale=TRUE)
          ...
          #Bootstrap AR
            for (bb in 1:B) {
                ...
                Ft_b[1]=ts(PC1)[1]
                for (rr in 2:t) {
                   Ft_b[rr]<-fit$ar*Ft_b[rr-1]+sqrt((1-fit$ar^2))*resid_boot[rr]
                ...
                Dif1<-colSums(abs(Ft_b-PC1ESTb))
                Dif2<-colSums(abs(Ft_b-PC1ESTINVb))
                if (Dif1<Dif2) {
                    PC1b=PC1ESTb
                  } else {
                      PC1b=PC1ESTINVb
            ...
            if (ar[bb]>1) {ar[bb]=0.99}
                for (tt in 2:t) {
                    Ft_boot[tt]<-ar[bb]*tPC1[tt-1]+sqrt((1-ar[bb]^2))*resid[tt]
                  }
                y_boot<-matrix(Loadb%*%t(Ft_boot)+Resid_boot,nrow=N,ncol=t)
              ...
              #Sacando PC del bootstrap
                ...
                #Efecto espejo
                  PC1ESTINV<-(-1)*PC1EST
                Dif1<-colSums(abs(Ft_boot-PC1EST))
                Dif2<-colSums(abs(Ft_boot-PC1ESTINV))
                if (Dif1<Dif2) {
                    PC1B=PC1EST
                  } else {
                      PC1B=PC1ESTINV
        ...
        lines(ts(CIL4),col="red")
        lines(ts(CIU4),col="red")
 ...
 write.csv2(salida,"M13  B=500 R=300  T=100 N=200.csv")